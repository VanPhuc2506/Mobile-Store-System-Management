...
public class ThongKeDAO {
    ...
    public double getInCome(Date start, Date end) {
        try {
            ...
            String d1 = formatter.format(start);
            String d2 = formatter.format(end);
            String sql = "SELECT SUM(TongTien) FROM HoaDon WHERE NgayLap BETWEEN CAST('"+d1+"' AS DATE) AND CAST('"+d2+"' AS DATE)";
    ...
    public double getSpend(Date date) {
        try {
            Format formatter;
            formatter = new SimpleDateFormat("yyyy-MM-dd");

            String d1 = formatter.format(date);
            String sql = "SELECT SUM(TongTien) FROM phieunhap WHERE NgayLap = '"+d1+"'";
            // String sql = "SELECT SUM(TongTien) FROM `phieunhap` WHERE NgayLap BETWEEN CAST('2021-01-24' AS DATE) AND CAST('2022-09-24' AS DATE) ";
            MyConnect Myconn = new MyConnect();
            PreparedStatement pre = MyConnect.conn.prepareStatement(sql);
    ...
    public double getInCome(Date date) {
        try {
            ...
            String d1 = formatter.format(date);
            String sql = "SELECT SUM(TongTien) FROM HoaDon WHERE NgayLap = '"+d1+"'";
            // String sql = "SELECT SUM(TongTien) FROM `phieunhap` WHERE NgayLap BETWEEN CAST('2021-01-24' AS DATE) AND CAST('2022-09-24' AS DATE) ";
            MyConnect Myconn = new MyConnect();
            PreparedStatement pre = MyConnect.conn.prepareStatement(sql);
            ResultSet rs = pre.executeQuery();
    ...
    public double getSpend(Date start, Date end) {
        try {
            ...
            String d1 = formatter.format(start);
            String d2 = formatter.format(end);
            String sql = "SELECT SUM(TongTien) FROM phieunhap WHERE NgayLap BETWEEN CAST('"+d1+"' AS DATE) AND CAST('"+d2+"' AS DATE)";
           // String sql = "SELECT SUM(TongTien) FROM `phieunhap` WHERE NgayLap BETWEEN CAST('2021-01-24' AS DATE) AND CAST('2022-09-24' AS DATE) ";
            MyConnect Myconn = new MyConnect();
    ...
    /*SELECT SUM(TongTien) AS RS FROM hoadon WHERE NgayLap BETWEEN CAST('2021-01-01' AS DATE) AND CAST('2021-02-01' AS DATE)*/
}